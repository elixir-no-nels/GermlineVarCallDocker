...
class GATK_Mutect_Chr_Loop < Toolbase
  ...
  def get_config_template
    erb_template = %~
     ...
     mutect_jar       = <%= @opt_parser.get      from: 'mutect_jar',       default_value: 'mutect.jar', required: false,  type: String, comment: 'path/binary.jar for mutect' %>
    ...
    template = %~
      ...
      for files in sample_group_list do
        ...
        for chr in chr_array do
          ...
          out_txt = "\#{output_dir}/\#{base_name}_chr_\#{chr}\#{output_suffix}.txt"
          out_vcf = "\#{output_dir}/\#{base_name}_chr_\#{chr}\#{output_suffix}.vcf"
