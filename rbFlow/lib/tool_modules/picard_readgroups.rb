      ...
      for file in files_list do
        ...
        if read_groups != false
          ...
          for tag in ['ID','SM','LB','PL','PU'] do
            rg[tag] = 'NotDefined'
            if (read_groups[tag].class == Integer or read_groups[tag].class == Array) and read_groups['split'].class == String
              splited_name = basename.split read_groups['split']
              rg[tag] = splited_name[read_groups[tag]] if read_groups[tag].class == Integer
              if read_groups[tag].class == Array
                rg[tag] = ''
                for element in read_groups[tag]
                  rg[tag] = rg[tag] + '_' + splited_name[element] if element.class == Integer
